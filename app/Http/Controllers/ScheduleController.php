...
class ScheduleController extends Controller
{
    public function index()
    {
        ...
        if (request()->ajax()) {
            $query = Schedule::select('schedule.*', 'course.short_name as course', 'section.name as section')
    ...
    {
        if (request()->ajax()) {
            $query = TimeSlot::select(DB::raw('count(schedule_id) as subject', 'schedule.*', 'time_slot.*', 'subjects.subj_code as subject_name'))
